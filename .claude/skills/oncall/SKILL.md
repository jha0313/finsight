...
### ③ 유저 질문 → 근거 있는 답  (Autopilot · 컨셉)
- FinSight **유저**(제품 도우미)와 **팀 내부**(운영 동료) 양쪽 질문에 **코드·PostHog 로그·Supabase read-only**라는 근거(grounding)로 답한다.
- 답하는 **두뇌**는 이 하네스로 이미 가능하다. 빠진 건 능력이 아니라 **항상 켜진 입구(always-on)** — 호스팅·세션·채널 인프라. 그건 후속(Part 4)에서 몸을 단다.
- 인터랙티브 one-shot으로 지금도 쓸 수 있다 (예: *"방금 들어온 이 에러, 유저한텐 뭐라고 답하지?"*). 근거 규칙: `references/service-map.md`.
...
- "이 에러 유저한테 뭐라고 답하지 / 어젯밤 prod 왜 났어" — ③ autopilot
