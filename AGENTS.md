...
codex 등 코딩 에이전트가 finsight 저장소에서 작업할 때 따르는 지침. codex는 이 파일을 작업 디렉토리에서 자동으로 읽는다.

> 이 파일은 `CLAUDE.md`(Claude Code용)의 코덱스 대응본이며 동일한 프로젝트 규칙을 담는다. **둘 중 하나를 고치면 다른 하나도 함께 갱신**하라. 전체 설계 근거는 `docs/PRD.md`, `docs/ARCHITECTURE.md`, `docs/ADR.md`, `docs/DESIGN.md` 참고.

## 프로젝트

임의의 카드/은행 명세서 CSV를 업로드하면 Claude로 분석해 지출 구조·이상 거래·절약 인사이트를 대시보드로 보여주는 글로벌 핀테크 SaaS MVP.

## 기술 스택
- Next.js 15 (App Router + Route Handlers)
- TypeScript strict mode
- Tailwind CSS
- 테스트: Vitest (`lib/`·`services/` 비즈니스 로직) · 하네스 스크립트(`scripts/`)는 pytest
...
## 개발 프로세스
- CRITICAL: 비즈니스 로직(`lib/`·`services/`)은 테스트를 먼저 작성하고, 통과하는 구현을 작성하라(TDD). 이는 `.codex/hooks.json`의 TDD 가드(아래 **Codex hooks** 참고)로도 강제되지만, 가드는 가드레일일 뿐이니 **규칙을 스스로 지켜라**.
- 각 작업은 `npm run lint && npm run build && npm run test`가 통과한 상태(green)로 끝내라.
- 위험한 명령(`rm -rf`, `git push --force`, `git reset --hard`, `DROP TABLE` 등)은 실행하지 마라.
...
## 하네스 (scripts/)
`scripts/execute.py`는 phase 디렉토리의 step을 순차 실행하는 자가-교정 하네스다. 각 step을 다음 명령으로 **codex**에 위임한다:

```
codex exec --json --dangerously-bypass-approvals-and-sandbox <prompt>
```

- `python3 scripts/execute.py <phase-dir> [--push]` 로 실행한다.
- `--dangerously-bypass-approvals-and-sandbox`는 승인 프롬프트·샌드박스를 건너뛴다(자동화 전용). 외부에서 통제된 환경에서만 사용하라.
- step별 산출물은 `phases/<dir>/step{N}-output.json`, 진행 상태는 `index.json`에 기록된다.
- 하네스 스크립트 자체를 수정할 때는 `python3 -m pytest scripts/test_execute.py`를 green으로 유지하라.

## Codex hooks (`.codex/hooks.json`)
프로젝트 가드는 codex 레이어인 `.codex/hooks.json`에 둔다(`.claude/settings.json`과 별개). 처음 한 번 `/hooks`에서 **review & trust** 해야 실행된다(미신뢰 시 skip). 자동화에서 신뢰를 건너뛰려면 `codex exec --dangerously-bypass-hook-trust`.

- `PreToolUse[Bash]` → `scripts/hooks/codex-block-dangerous.sh`: `rm -rf`·`git push --force`·`git reset --hard`·`DROP TABLE` 차단.
- `PreToolUse[Edit|Write|apply_patch]` → `scripts/hooks/codex-tdd-guard.sh`: apply_patch 패치에서 대상 파일을 추출해 공유 `tdd-guard.sh`에 위임(테스트 없는 `lib/`·소스 구현 차단).
- `Stop` → `scripts/hooks/codex-verify-gate.sh`: lint/build/test 실패 시 `decision:block`으로 codex를 이어 자가교정(통과 시 정상 종료). **`package.json`이 없으면(스캐폴딩 전) 게이트와 TDD 가드는 건너뛴다.**
