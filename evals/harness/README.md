...
한 줄·프롬프트 한 문장·스킬 한 줄이 에이전트 행동을 바꾸므로, 하네스도 코드처럼
회귀한다. 세 축을 본다:
...
- **oncall 트랙** — `.claude/skills/oncall/` 스킬이 prod alert를 노이즈/신호로
  올바로 가르고, 신호면 분석을 담아 escalation하며, 노이즈면 사람을 안 깨우는지
  (트리거 ② 1차 대응의 핵심). `triage-rubric.md`의 '알려진 일시적' 한 줄을
  지우면 해당 노이즈 케이스가 회귀로 잡힌다.
...
- **golden set**(`cases/*.md`): 입력 + 기대 라벨. `kind: review|qa|oncall`로 트랙을 가른다.
  - review(5): 본문=코드, 라벨=`expect`/`severity`/`rule`. 위반 4 + 정상(오탐 방지) 1.
  - qa(4): 본문=질문, 라벨=`must`(반드시 담겨야 할 사실)·`must_not`(말하면 안 되는 오답).
    사실 검증 3 + 전제반박 가드 1.
  - oncall(5): 본문=prod alert + 운영 컨텍스트, 라벨=`triage`(noise|signal)·`must`·`must_not`.
    신호 3(결제 새 에러·급증·경계→신호 가드) + 노이즈 2(일시적 타임아웃·봇 스캐너).
...
- **분류기**(`triager.ts`): oncall 트랙 대상. **라이브 oncall 스킬**(SKILL.md +
  triage-rubric + service-map)을 컨텍스트로 받아 alert를 노이즈/신호로 분류하는 경량
  단일 호출(Sonnet, temperature 0). 산문 판정을 출력한다.
- **judge**(`judge.ts`): 다른 모델(Opus)이 "subject가 기대대로 했나"를 pass/fail로
  채점한다(`judge`=review, `judgeQa`=qa, `judgeOncall`=oncall).
...
evals/harness/
  ...
  triager.ts              oncall 트랙 대상 — 라이브 oncall 스킬 컨텍스트 (Claude 호출)
  judge.ts                LLM-as-judge: judge(review) / judgeQa(qa) / judgeOncall(oncall)
