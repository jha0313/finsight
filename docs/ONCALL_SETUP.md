...
트리거 ③ 유저 질의 (Autopilot · 컨셉)
  답하는 두뇌는 oncall 스킬로 이미 가능. always-on 입구(호스팅/채널)는 후속.
```
...
- **트리거 ③ Autopilot**(로컬 one-shot): 터미널에서 oncall 스킬로 *"방금 들어온 이 에러, 유저한텐 뭐라고 답하지?"* 질문 → 코드·로그·DB 근거로 답.
